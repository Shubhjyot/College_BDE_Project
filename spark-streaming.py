...
if __name__ == "__main__":
    ...
    total_votes_to_kafka = (
        total_votes.selectExpr("to_json(struct(*)) AS value")
        .writeStream.format("kafka")
        .option("kafka.bootstrap.servers", "localhost:9092")
        .option("topic", "aggregated_total_votes")
        .option("checkpointLocation", os.getenv('CHECKPOINT1'))
        .outputMode("update")
        .start()
    )

    votes_per_candidate_to_kafka = (
        votes_per_candidate.selectExpr(
            "to_json(struct(*)) AS value"
        )  # The result is a DataFrame with a single column named 'value' containing JSON-encoded strings.
        .writeStream.format("kafka")
        .option("kafka.bootstrap.servers", "localhost:9092")
        .option("topic", "aggregated_votes_per_candidate")
        .option("checkpointLocation", os.getenv('CHECKPOINT2'))
        .outputMode("update")
        .start()
    )
    ...
    turnout_by_location_to_kafka = (
        ...
        .option("checkpointLocation", os.getenv('CHECKPOINT3'))
        .outputMode("update")
        .start()
    )
